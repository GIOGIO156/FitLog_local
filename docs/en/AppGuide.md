...
- The body-profile calendar entry is separate from current Profile editing: it opens the app date picker with today selected by default, enters an inline card state for past dates, and edits only that date's weight, body-fat, and waist body metric record. Selecting today exits history editing and returns to the current Profile card. Other Profile areas and bottom navigation are softened and locked during the past-date state.
- Existing past body metric records show a compact delete affordance next to the date pill; deletion requires confirmation and removes that dated body metric record, including the mirrored weight log when the record had a weight value. The confirmation dialog keeps the normal theme surface, keeps Cancel as an ordinary text action, and uses an unfilled red icon-text action for Delete so destructive intent is clear without conflicting with the app's green save/confirm affordances.
...
- The Body Trend card is read-only, switches between weight, body fat, and waist, does not contain a full body-history list, and keeps the chart clean until the user taps a point for its date/value tooltip. Point spacing starts from the first visible record and scales by the selected 7/14/21/28 day range. Horizontal reference lines come from readable metric tick intervals, such as 1 kg/cm/% for small ranges and larger 2/5/10-style intervals for wider ranges, but axis labels stay hidden. If today has no dated body metric row, the chart uses the current saved Body Profile snapshot as today's display point without writing a history row.
