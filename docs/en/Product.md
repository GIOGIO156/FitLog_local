...
21. On Android, an active strength draft can also show a persistent workout notification. The notification title is only the current exercise name, the body shows the next set such as `Set 2 of 9 - 50 kg x 8 reps`, the notification large icon uses the current transparent exercise image directly, and the status-bar small icon comes from the saved transparent FitLog SVG source converted into an Android drawable.
...
9. Theme and language preferences are low-frequency local UI settings near the lower Profile settings area; theme choices are Green, Blue, and Black, and they only remap app color tokens.
   ...
   The Black theme keeps the black-orange direction with dark background, card, input, selected-surface, and outline roles separated so orange remains an accent instead of becoming a general surface color. In the workout library, filter chips use rounded dark surfaces with orange selected labels, and exercise thumbnails use low-glare circular fills plus subtle body-part rings so body-part identity remains visually distinct while the exercise artwork stays legible on dark cards. The transparent PNG assets remain unchanged; the fill/ring treatment is applied by runtime rendering.
   Black-theme workout-library thumbnails use the finalized body-part palette: Chest `#402824`/`#8B5A52`, Back `#27303D`/`#60718A`, Legs `#41341C`/`#9A7D40`, Glutes `#D66A3C`/`#F1A073`, Shoulders `#626A72`/`#9AA4AD`, Arms `#203838`/`#5D8989`, Core `#402830`/`#8B5A68`, Cardio `#21382D`/`#5F8B70`, and Full Body `#68727E`/`#A0AABD`; the first color is the inner fill and the second is the thin outer ring.
