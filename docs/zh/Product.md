...
15. 用户通过应用返回键或系统返回手势离开编辑页时，会先保存并保留草稿，而不是强制弹出保存/舍弃弹窗，然后清除轻量自动恢复标记。
16. 如果用户仍停留在新建 Add Workout 草稿编辑页，且 App 进入 `inactive`、`paused` 或 `hidden`，FitLog 会立即持久化 SQLite 草稿，并在 SharedPreferences 中保留一个“编辑器仍 active”的轻量标记。这个标记只决定是否自动导航，SQLite 草稿仍是唯一权威数据。
17. 冷启动时，Root 只检查一次：如果标记存在、SQLite 中的 active draft 是新建训练草稿，且 `updatedAt` 距启动不超过 30 分钟，FitLog 会切到 Workout 并只打开一次 Add Workout，恢复原日期、动作、组行、重量、次数或时长、完成状态、记录名和备注。
18. 30 分钟只是冷启动恢复资格窗口。FitLog 不运行后台倒计时、Alarm、Worker、前台服务、WakeLock、保活机制或任何后台计算来支持这个恢复路径。
19. 用户主动退出编辑页、舍弃草稿、成功保存正式记录，以及草稿被清空或删除时，都会清除标记。正常退出后或超过 30 分钟后，SQLite 草稿仍可从 Workout Log 的草稿条手动恢复，不会被自动打开或删除。
20. Workout Log 会在 `添加训练` 上方显示一条紧凑的双行草稿恢复条；标题优先显示训练记录名，否则回退为 `训练草稿`，副标题使用短部位名，最多直接显示三个部位，超过后改为 `+n`，然后再拼接动作数量摘要，或在还没有动作时显示 `点击继续编辑`。
21. 在 Android 上，活动中的力量草稿可以显示一条常驻训练通知。通知标题只显示当前动作名，正文显示下一组，例如 `第 2 组，共 9 组 - 50 kg x 8 次`，通知 large icon 直接使用当前动作的透明图片，状态栏小图标来自保存的透明 FitLog SVG 源文件并转换为 Android drawable。
22. 训练通知按最近一次勾选完成的组决定焦点：如果该动作还有未完成组，就继续提示该动作的下一组；如果该动作已经全部完成，就按训练记录顺序找第一项还有未完成组的动作。所有力量组都完成后，通知进入“返回保存训练”的完成提示。
23. 点击 Android 通知主体会通过与 Workout Log 草稿条一致的恢复路径打开 Add/Edit Workout Record。系统展开箭头仍由 Android 控制，不是 App 自定义 action。
24. 只有用户显式点击保存且校验通过后，才会写入正式训练记录。
25. 力量训练保存时只持久化已完成组；未勾选组会被移除，保存后的组号按 `1..n` 重排。
26. 一条多动作记录存储为多条共享同一 `plan_id` 的 `workout_sessions`；每条 session 也保存相同的 `record_name`。
27. 保存后的记录保留动作快照，所以之后修改可复用自定义动作不会重新解释历史记录。
28. 保存后的记录展示总时长、计算口径训练量、总组数、估算消耗和动作卡片。
29. 编辑已保存记录时，正式保存仍会以事务替换整个 `plan_id` 分组；未保存改动在用户保存或舍弃前只停留在草稿层。编辑已有历史训练产生的草稿不参与新建训练的冷启动自动恢复路径。
...
3. 身体资料摘要与统一编辑：年龄、身高、体重、性别、体脂率和腰围保持可读的展示网格；用户点某一项时整个身体资料网格进入当前 Profile 编辑态，有改动时通过同一卡片内的一次保存/取消完成，而不是跳到单独长表单。
...
9. 主题和语言偏好是低频本地 UI 设置，放在 Profile 较靠后的设置区域；主题选项为绿色、蓝色和黑橙，只重新映射 App 颜色 token。
   ...
   黑色主题保持黑橙方向，同时拆分深色背景、卡片、输入、选中表面和描边角色，让橙色继续作为强调色，而不是变成通用表面色。训练动作库中，筛选 chips 使用深色圆角表面和橙色选中文案，动作缩略图使用低眩光圆形内圈底色加轻量部位外圈，让部位身份肉眼可分，同时保证动作图案在深色卡片上仍清楚可读。透明 PNG 资产本身保持不变；内圈和外圈由运行时渲染产生。
   黑色主题动作库缩略图使用定稿的按部位配色：胸部 `#402824`/`#8B5A52`，背部 `#27303D`/`#60718A`，腿部 `#41341C`/`#9A7D40`，臀部 `#D66A3C`/`#F1A073`，肩部 `#626A72`/`#9AA4AD`，手臂 `#203838`/`#5D8989`，核心 `#402830`/`#8B5A68`，有氧 `#21382D`/`#5F8B70`，全身 `#68727E`/`#A0AABD`；每组颜色前者是内圈实色，后者是细外圈色。
...
- 本地主题切换
- 二次确认后清空本地数据
- 活动中新建训练草稿的 30 分钟冷启动自动恢复，基于 SQLite 草稿数据和一个轻量本地标记
