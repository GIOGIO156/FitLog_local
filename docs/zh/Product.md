...
9. 有氧强度使用可维持时长表示：60 分钟以上、30-60 分钟、10-30 分钟、3-10 分钟，或小于 3 分钟且需要休息。
10. 间歇或极高强度有氧会记录实际运动时长，避免把休息时间按极高强度计算。
...
15. Workout Log 会在 `添加训练` 上方显示一条紧凑的双行草稿恢复条；标题优先显示训练记录名，否则回退为 `训练草稿`，副标题使用短部位名，最多直接显示三个部位，超过后改为 `+n`，然后再拼接动作数量摘要，或在还没有动作时显示 `点击继续编辑`。
16. 只有用户显式点击保存且校验通过后，才会写入正式训练记录。
17. 力量训练保存时只持久化已完成组；未勾选组会被移除，保存后的组号按 `1..n` 重排。
18. 一条多动作记录存储为多条共享同一 `plan_id` 的 `workout_sessions`；每条 session 也保存相同的 `record_name`。
19. 保存后的记录保留动作快照，所以之后修改可复用自定义动作不会重新解释历史记录。
20. 保存后的记录展示总时长、计算口径训练量、总组数、估算消耗和动作卡片。
21. 编辑已保存记录时，正式保存仍会以事务替换整个 `plan_id` 分组；未保存改动在用户保存或舍弃前只停留在草稿层。
