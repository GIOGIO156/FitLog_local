...
- 身体资料卡的日历入口与当前 Profile 编辑分离：它先打开仅允许过去日期的应用日期选择器，确认后进入卡片内编辑态，并只编辑该日期的体重、体脂率和腰围身体指标记录；编辑期间其它 Profile 区域和底部导航会柔和淡化并锁定。
- Profile 内联数字编辑器使用透明的卡片内输入框并保持 tile 尺寸稳定；键盘弹出时，当前焦点输入可滚动到键盘上方，底部导航保持固定。
- 身体趋势卡是只读展示层，可切换体重、体脂率和腰围，不承载完整历史记录列表；图表默认保持干净，只有点按记录点时显示日期和值。
