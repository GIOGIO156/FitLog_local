...
- 每个内置动作都有专属透明 PNG 缩略图；自定义动作或其它未匹配动作仍回退到按身体部位区分的共享 SVG 图标。
- 在黑色主题下，动作库使用深色圆角身体部位筛选 chips，并为动作缩略图使用按部位区分的低眩光圆形内圈底色加轻量部位外圈，既保证不同部位肉眼可分，也保证动作图案在深色卡片上仍然清楚；精确配色见 Product。
...
- 通知 large icon 直接使用当前动作的透明图片。状态栏小图标使用由 `assets/icons/app/fitlog_notification_small.svg` 这个透明 FitLog SVG 源文件转换出的 Android drawable，但 Android 仍可能把这个位置渲染成系统控制的单色图标。
...
- 黑色主题让橙色保持强调用途，同时拆分深色背景、卡片、选中表面、输入、描边和文字角色，保证层级更清楚。动作库筛选 chips 使用深色圆角表面和橙色选中文案，动作缩略图使用按部位区分的低眩光圆形内圈底色加轻量部位外圈来保证深色卡片上的图案对比度，并避免不同部位在肉眼上撞色。透明 PNG 资产本身保持不变；内圈和外圈由运行时渲染产生，精确配色记录在 Product。
